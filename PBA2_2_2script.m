...
% substitute G(alpha, v) = alpha*T/m, where G > 0 as 
% T = lambda*Tm*(1-beta*(alpha*v/wm -1)^2), any change of 
% coordinates will still result in G > 0, as v is squared in T.
% the difference between G(alpha, v=x+Vr) is only noticable in magnitude
% therefore, with this condition in mind:
syms G

F_vz = subs(F_vz, alpha*T/m, G);
% evaluate a change in coordinates of F(x)
syms x y
F_vy = simplify(subs(F_vz, z, y + (Fd/(G*Ki))));    
